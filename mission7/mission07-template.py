...
def get_station_position(line, station_code):
    ...
    if station:
        return line[1].index(station)#using index method to get index
    return -1
...
def get_direction(line, train_position):
    ...
    from_station = get_from_station(train_position)
    to_station = get_to_station(train_position)
    from_position = get_station_position(line, get_station_code(from_station))
    to_position = get_station_position(line, get_station_code(to_station))
...
def parse_events_in_line(data_file, line):
    ...
    for row in rows:
        ...
        is_moving_string = row[1]
        is_moving = True
        if is_moving_string =="TRUE":
            is_moving = True
        else:
            is_moving = False
        ...
        train_dt = date + ' '+ time
        event_dt = datetime.datetime.strptime(train_dt, '%d/%m/%Y %H:%M')
        schedule = make_schedule_event(train, train_position, event_dt)
...
def is_valid_event_in_line(bd_event, line):
    ...
    from_position = get_station_position(line, get_station_code(from_station))
    to_position = get_station_position(line, get_station_code(to_station))
    travel = from_position - to_position
    time = dt.time()
    
    
    return abs(travel) == 1 and datetime.time(7,0)<=time<=datetime.time(23,0)
...
def get_location_id_in_line(bd_event, line):
    ...
    if is_moving:
        location_id = min(from_position, to_position) + 0.5
    else:
        location_id = from_position
    return location_id
...
def get_schedules_at_time(train_schedule, time):
    ...
    for event in train_schedule:
        datetime = get_schedule_time(event)
        if datetime == time:
            sche_event += (event,)
    return sche_event
...
def calculate_blame_in_line(full_schedule, valid_bd_events, line, scorer):
    for j in valid_bd_events:
        time = get_schedule_time(j)
        location = get_location_id_in_line(j, line)
        one = get_rogue_schedules_in_line(full_schedule, line, time, location)
        ...
        def filter_train(schedules):
            ...
            return result
        for k in filter_train(one):
            scorer = blame_train(scorer, k)
    return scorer
