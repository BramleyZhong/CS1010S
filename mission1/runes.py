...
def simple_fractal(params):
    # Fill in code here
    return(beside(params,stack(params,params)))

show(simple_fractal(make_cross(rcross_bb)))
clear_all()
show ( simple_fractal ( heart_bb ))
clear_all()
def repeat_pattern(n,pic):
        if n == 0:
                return pic
        else:
                return repeat_pattern(n-1,pic)

show(repeat_pattern(4, make_cross(nova_bb)))
clear_all()
show(repeat_pattern(8, make_cross(nova_bb)))
clear_all()
def egyptian1(pic, n):
        
            side1=stack_frac(1/n,quarter_turn_right(stack_frac(1/n,blank_bb,stackn(n-1,quarter_turn_left(pic)))),blank_bb)
            side2=quarter_turn_right(stack_frac(1/n,quarter_turn_right(stack_frac(1/n,blank_bb,stackn(n-1,quarter_turn_right(quarter_turn_right(pic))))),blank_bb))
            side3=quarter_turn_right(quarter_turn_right(stack_frac(1/n,quarter_turn_right(stack_frac(1/n,quarter_turn_right(blank_bb),stackn(n-1,quarter_turn_right(pic)))),blank_bb)))
            side4=quarter_turn_left(stack_frac(1/n,quarter_turn_right(stack_frac(1/n,blank_bb,stackn(n-1,pic))),blank_bb))
            return scale(1-2/n,pic)

def egyptian(pic, n):
        return stack_frac(1/n,quarter_turn_right(stackn(n,quarter_turn_left(pic))),
               stack_frac((n-2)/(n-1),quarter_turn_right(stack_frac(1/n,quarter_turn_left(stackn(n-2,pic)),stack_frac((n-2)/(n-1),quarter_turn_left(pic),quarter_turn_left(stackn(n-2,pic))))),
               quarter_turn_right(stackn(n,quarter_turn_left(pic)))))
...
def steps(pic1,pic2,pic3,pic4):
        return mosaic(overlay_frac(3/4, blank_bb, pic1),
                      overlay_frac(1/2, blank_bb, pic2), overlay_frac(1/4, blank_bb, pic3), pic4)
def mosaic(pic1, pic2, pic3, pic4):
        return stack(beside(pic4,pic1), beside(pic3,pic2))
...
def dual_fractal_iter(pic1, pic2, n):
        ...
        if n%2 == 0:
                another = stackn(2**(n-1), pic2)
        elif n%2 == 1:
                another = stackn(2**(n-1), pic1)
                        
        for i in range(n-1, 1, -1):
                if i%2 == 0:
                        another = beside(stackn(2**(i-1), pic2), another)
                elif i%2 == 1:
                        another = beside(stackn(2**(i-1), pic1), another)
                                
        return beside(pic1, another)
...
show(fifty_shades_of_gray(create_tile()))
clear_all()

from random import *

def get_pat(n=25, pic=circle_bb):
    res = pic
...
def abstract_art(coef):
    res = coef
    for i in range(9):
        x,y = uniform(-1,1), uniform(-1,1)
        layer = translate(x, y, coef)
        res = overlay_frac(1/4, layer, res)
    return res

coef = get_pat()
show(abstract_art(coef))
clear_all()

def create_sth(n=10, pic=black_bb):
    pic = scale_independent(1, 1/5, nova_bb)
    res = translate(0, 0.38, pic)
    y = 0.38
    for i in range(13):
        res = overlay_frac(2/9, translate(0,y,pic), res)
        y -= 0.1
...
def fifty_shades_of_gray(sth, n=4):
    if n == 1:
        return sth
    return stack(beside(sth, sth), fifty_shades_of_gray(stackn(n-1, sth), n-1))
    
hollusion(fifty_shades_of_gray(create_sth()))
